...
for number in numbers:
    ...
    for board in range(len(boards)):
        ...
        # Skipping boards that already won
        if board in victorious_boards:
            continue
        ...
        if any(row_win) or any(col_win):
            ...
            # Just one remaining board
            if len(victorious_boards) == len(boards) - 1:
            
                # Getting sum of all non-marked entries
                res = 0
                for row in range(5):
                    for col in range(5):
                ...
                victorious_boards.append(board)
